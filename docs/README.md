...
Feature 1 uses `Asia/Manila` as the application timezone and treats 9:00 PM as the daily overdue cutoff. The Vercel cron at `13:00 UTC` invokes `/api/keepalive` daily. Configure `SUPABASE_SERVICE_ROLE_KEY` in Vercel so the cron can call `mark_overdue_transactions()` in the background. The browser calls the function only for active Student Assistant sessions; this is a fallback when the service-role cron key is unavailable. Clinical Instructor sessions do not call this SA-only RPC.

Feature 1 test records are created only by `database/09-feature-1-test-data.sql` and removed by `database/09-remove-feature-1-test-data.sql`. The seed includes one available fake item for submitting a borrow request with the current automatic two-day due date. Do not run the seed against production without explicitly recording the target database and test run.
